...
#define MAX_UCTX_PER_CORE 16384
...
static int g_pattern_matching = 0;
static int g_monitor_len = 64 * 1000;
static int g_measure_delay = 0;
static int g_write_all = 0;
static int g_cnt_conn = 0;
static FILE *g_delay_fp;
static char g_path[MAX_PATH_LEN] = FILE_PATH;
static char g_delay_filename[MAX_PATH_LEN] = DELAY_FILENAME;
static mmctx_t g_mmctx[MAX_CPUS];
static struct rte_mempool *g_uctx_pool[MAX_CPUS] = {NULL};
/*---------------------------------------------------------------------------*/
static inline void
...
cb_session_start(mmctx_t mmctx, int cid, int side)
{
	...
	if ((c = mmtls_get_uctx(mmctx, cid)))
		...
		return;
	if (rte_mempool_get(g_uctx_pool[mmctx->cpu], (void **)&c) < 0)
...
cb_session_end(mmctx_t mmctx, int cid, int side)
{
	...
	rte_mempool_put(g_uctx_pool[mmctx->cpu], c);
...
int main(int argc, char **argv)
{
	...
	{
		char name[MAX_POOL_NAME_LEN + 1];
		sprintf(name, "uctx-pool-%d", i);
		g_uctx_pool[i] = rte_mempool_create(name,
											MAX_UCTX_PER_CORE,
											sizeof(uctx),
											...
											0);
		if (!g_uctx_pool[i])
			EXIT_WITH_ERROR("rte_mempool_create failed");
	...
	/* destroy mempool */
	for (i = 0; i < core_num; i++)
		rte_mempool_free(g_uctx_pool[i]);
