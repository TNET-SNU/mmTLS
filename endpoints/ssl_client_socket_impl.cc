...
/*---------------------------------------------------------------------------*/
static inline int 
read_hex(uint8_t *dst, const char *src, int max)
{
  int len = 0;
                        ...
                        uint8_t* c_iv, uint8_t* s_iv)
{
  ...
  memcpy(s_iv, p + key_len + key_len + EVP_GCM_TLS_FIXED_IV_LEN, EVP_GCM_TLS_FIXED_IV_LEN);
}
/*---------------------------------------------------------------------------*/
static inline void
...
destroy_key_channel(SSL_CTX *ssl_ctx)
{
  ...
  if (!channel)
    return;
  key_ctx = SSL_get_SSL_CTX(channel->key_ssl);
  if (!key_ctx)
    return;
  pthread_mutex_destroy(&channel->mutex);
  key_ssl = channel->key_ssl; 
  if (!key_ssl)
    return;
...
init_key_channel(SSL_CTX *ssl_ctx)
{
  ...
  if (!(channel = (key_chan *)calloc(1, sizeof(key_chan)))) {
    perror("calloc");
    return -1;
  }
  ...
  SSL_CTX_set_options(key_ctx, SSL_OP_NO_TICKET);
  ...
  if (!(channel->key_ssl = SSL_new(key_ctx))) {
    ERR_print_errors_fp(stderr);
    goto err;
  }
  /* TCP connection */
  ...
  if (setsockopt(key_chan_sd, SOL_SOCKET, SO_REUSEPORT, &optval, sizeof(optval)) < 0) {
    perror("setsockopt");
    goto err;
  }
  ...
  if (connect(key_chan_sd, (struct sockaddr *)&svraddr, sizeof(svraddr)) < 0) {
    perror("connect");
    goto err;
  }
  if (SSL_set_fd(channel->key_ssl, key_chan_sd) < 0) {
    ERR_print_errors_fp(stderr);
    goto err;
  }
  if (SSL_connect(channel->key_ssl) < 0) {
    ERR_print_errors_fp(stderr);
    goto err;
  }
  printf("connected!\n");
...
keysend_callback(const SSL *ssl, const char *line)
{
  ...
  /* line parsing start */
	if ((res = sscanf(line, "CLIENT_TRAFFIC_SECRET_0 %s %s", crandom_buf, secret_buf)) > 0)
		flag = CLIENT_SIDE; /* tls1.3 */
	else if ((res = sscanf(line, "SERVER_TRAFFIC_SECRET_0 %s %s", crandom_buf, secret_buf)) > 0)
		flag = SERVER_SIDE; /* tls1.3 */
  else if ((res = sscanf(line, "CLIENT_RANDOM %s %s", crandom_buf, secret_buf)) > 0)
		flag = TLS12; /* tls1.2 */
	else
  ...
  *(uint16_t *)ptr = htons(SSL_CIPHER_get_protocol_id(ssl_cipher));
  ptr += sizeof(uint16_t);
  *(uint16_t *)ptr = htons(key_len + iv_len + mac_key_len);
  ptr += sizeof(uint16_t);
  ...
  do {
    bytes = SSL_write(channel->key_ssl, payload + offset, BUF_SIZE - offset);
    if (bytes <= 0)
      continue;
    offset += bytes;
