...
#if MMTLS
    TAG2_CTX *ctx2 = (TAG2_CTX *)EVP_CIPHER_CTX_get_app_data(ciph_ctx);
    if (!ctx2) {
        /* first step of second CIPHER_CTX */
        ctx2 = calloc(1, sizeof(TAG2_CTX));
        /* ToDo: set below as negotiated one during handshake */
        memset(ctx2->key, 1, EVP_CIPHER_CTX_get_key_length(ciph_ctx));
        memcpy(ctx2->iv, iv, EVP_CIPHER_CTX_get_iv_length(ciph_ctx));
        EVP_CIPHER_CTX_set_app_data(ciph_ctx, ctx2);
    }
#endif
...
#if MMTLS
        /* Add tag 2 */
        uint8_t tag2[EVP_GCM_TLS_TAG_LEN] = {0};
        TAG2_CTX *ctx2 = (TAG2_CTX *)EVP_CIPHER_CTX_get_app_data(ctx);
        /* tag2 gen ctx should be in place !! */
        if (!ctx2) {
            RLAYERfatal(rl, SSL_AD_INTERNAL_ERROR, ERR_R_INTERNAL_ERROR);
            return 0;
        }
        tag2gen(ctx2->key, EVP_CIPHER_CTX_get_key_length(ctx),
                ctx2->iv, EVP_CIPHER_CTX_get_iv_length(ctx),
                recheader, sizeof(recheader),
                rec->data + rec->length - EVP_GCM_TLS_TAG_LEN,
                EVP_GCM_TLS_TAG_LEN, tag2);
#endif
    }
