...
#if MMTLS
void tag2gen(uint8_t *key, int keylen,
            uint8_t *iv, int ivlen,
            uint8_t *aad, int aadlen,
            uint8_t *cipher, uint64_t len,
            uint8_t *tag)
{
    CRYPTO_DATA H, EK0, Xi, Yi, AAD;
    AES_KEY aes_key;
    ...
    /* H = E(K, 0^{128}) */
    aesni_set_encrypt_key(key, keylen << 3, &aes_key);
    ...
    /* EK0 = E(K, Y) */
    memcpy(Yi.c, iv, ivlen);
    Yi.c[15] = 1;
    (*(block128_f)aesni_encrypt)(Yi.c, EK0.c, &aes_key);

    /* X_i =  */
    memcpy(AAD.c, aad, aadlen);
    ...
    /* bitlen = (40 || len(C)) */
    bitlen.hi = BSWAP8(5 << 3);
    bitlen.lo = BSWAP8(len << 3);
    memcpy(Xn + mres, &bitlen, sizeof(bitlen));
    mres += sizeof(bitlen);

    /* Xi = ( Xn xor bitlen ) dot H */
