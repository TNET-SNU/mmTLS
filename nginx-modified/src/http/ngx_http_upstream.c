...
#if (HYPERSCAN)
        u->state->dctx = open_DPI_context(sched_getcpu());
#endif
    }
        ...
        if (size && upstream->read->ready) {
            ...
            if (n > 0) {
                ...
                if (!u->state->stop_dpi) {
                    ...
                    over = u->state->response_length - target_bytes;
                    if (over > 0) {
                        ret = DPI(u->state->dctx, (char *)b->last, n - over);
                        u->state->stop_dpi = 1;
                    }
                    else {
                        ret = DPI(u->state->dctx, (char *)b->last, n);
                    }
                    /* DPI fail case.. should I exit? */
                    if (ret < 0) {
                        ngx_http_upstream_finalize_request(r, u, NGX_ERROR);
                        return;
                    }
...
#if (HYPERSCAN)
    close_DPI_context(u->state->dctx);
#endif
